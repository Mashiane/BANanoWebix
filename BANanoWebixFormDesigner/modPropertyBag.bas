...
Sub getPropertyBag As WixForm
	...
	tbl.CreateIcon("add_fields").SetIcon("mdi mdi-table-of-contents").SetTooltip("Add multiple fields").SetClick(BANano.CallBack(pgFormDesigner,"add_fields",Null)).Pop
	tbl.CreateIcon("download").SetIcon("mdi mdi-cloud-download-outline").SetTooltip("Download source code").SetClick(BANano.CallBack(pgFormDesigner,"download",Null)).Pop
	tbl.AddSpacer
