...
Sub btnsavegodown_click
	...
	Select Case Mode
		Case "A"
			...
			sqlite.Initialize
			sqlite.SetDB(dbName)
			sqlite.AddIntegers(Array("wh_no"))
			sqlite.AddStrings(Array("wh_name"))
			Dim rs As SQLiteResultSet1 = sqlite.Insert("godown", rec)
			rs.Result = BANano.FromJSON(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rs)))
		Case "E"
			...
			sqlite.AddIntegers(Array("wh_no"))
			sqlite.AddStrings(Array("wh_name"))
			Dim rs As SQLiteResultSet1 = sqlite.UpdateWhere("godown", rec, CreateMap("wh_no":wh_no))
			rs.Result = BANano.FromJSON(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rs)))
	End Select
	Mode = "E"
...
Sub Readgodown(wh_no As String)
	...
	sqlite.AddIntegers(Array("wh_no"))
	sqlite.AddStrings(Array("wh_name"))
	Dim rs As SQLiteResultSet1 = sqlite.Read("godown", "wh_no", wh_no)
	rs.Result = BANano.FromJSON(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rs)))
...
Sub godown_delete(did As String)
	...
	sqlite.AddIntegers(Array("wh_no"))
	sqlite.AddStrings(Array("wh_name"))
	Dim rs As SQLiteResultSet1 = sqlite.DeleteWhere("godown", CreateMap("wh_no":did))
	rs.Result = BANano.FromJSON(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rs)))
	Mode = ""
...
Sub dtgodown_afteredit(state As Object, editor As Object)
	...
	sqlite.AddIntegers(Array("wh_no"))
	sqlite.AddStrings(Array("wh_name"))
	Dim rs As SQLiteResultSet1 = sqlite.UpdateWhere("godown", item, CreateMap("wh_no":wh_no))
...
Sub LoadDataTable
	...
	Dim rsd As SQLiteResultSet1 = sqlite.SelectAll("godown", Array("*"), Array("wh_no"))
	Log(rsd)
	Log(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rsd)))
	
	rsd.Result = BANano.FromJSON(BANano.CallInlinePHPWait("BANanoSQLite1", sqlite.Build(rsd)))
...
Sub CreateForm As WixForm
	...
	formgodown.Initialize("formgodown")
	...
	wh_no.SetInvalidMessage("The no should be specified")
