...
Sub btnnewgodown_click
	...
	'initialize bananosql helper class
	Dim alaSQL As BANanoAlaSQL
	alaSQL.Initialize
	'get the maximum value in the primary field
	Dim rs As AlaSQLResultSet = alaSQL.GetMax("godown", "wh_no")
	rs.Result = warehouse.ExecuteWait(rs.query, rs.args)
	'increment the max value by 1
	Dim nextID As String = alaSQL.GetNextID("wh_no", rs.result)
...
Sub btnsavegodown_click
	...
	Select Case Mode
		Case "A"
			...
			Dim alaSQL As BANanoAlaSQL
			alaSQL.Initialize
			Dim rs As AlaSQLResultSet = alaSQL.Insert("godown", rec)
			rs.Result = warehouse.ExecuteWait(rs.query, rs.args)
		Case "E"
			...
			Dim alaSQL As BANanoAlaSQL
			alaSQL.Initialize
			Dim rs As AlaSQLResultSet = alaSQL.UpdateWhere("godown", rec, CreateMap("wh_no":wh_no))
			rs.Result = warehouse.ExecuteWait(rs.query, rs.args)
	End Select
	Mode = "E"
...
Sub Readgodown(wh_no As String)
	...
	Dim alaSQL As BANanoAlaSQL
	alaSQL.Initialize
	Dim rs As AlaSQLResultSet = alaSQL.Read("godown", "wh_no", wh_no)
	rs.result = warehouse.ExecuteWait(rs.query, rs.args)
...
Sub godown_delete(did As String)
	...
	Dim rs As AlaSQLResultSet = alaSQL.DeleteWhere("godown", CreateMap("wh_no":did))
	rs.Result = warehouse.ExecuteWait(rs.query, rs.args)
	Mode = ""
...
Sub dtgodown_afteredit(state As Object, editor As Object)
	...
	Dim alaSQL As BANanoAlaSQL
	alaSQL.Initialize
	Dim rs As AlaSQLResultSet = alaSQL.UpdateWhere("godown", item, CreateMap("wh_no":wh_no))
	rs.Result = warehouse.ExecuteWait(rs.query, rs.args)
...
Sub LoadDataTable
	...
	'add the data
	alaSQL.Initialize
	Dim rsd As AlaSQLResultSet = alaSQL.SelectAll("godown", Array("*"), Array("wh_no"))
	rsd.Result = warehouse.ExecuteWait(rsd.query, rsd.args)
