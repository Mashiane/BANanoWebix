...
'Static code module
Sub Process_Globals
	Type Period (Years As Int, Months As Int, Days As Int, Hours As Int, Minutes As Int, Seconds As Int)
	Private daysList As List
	Private monthsList As List
...
Private Sub PeriodBetweenHelper(Start As Long, EndTime As Long, StartWithDays As Boolean) As Period
	Dim td As Period
	td.Initialize
	Dim sy As Int = DateTime.GetYear(Start)
	Dim ey As Int = DateTime.GetYear(EndTime)
	Dim daysOffset As Int
	If EndTime > Start Then
		If Not(StartWithDays) Then
			'years
			If DateTime.Add(Start, ey - sy, 0, 0) > EndTime Then 
				td.Years = ey - sy - 1 
			Else 
				td.Years = ey - sy
			End If
...
Public Sub SetDateAndTime(Years As Int, Months As Int, Days As Int, Hours As Int, Minutes As Int, Seconds As Int) As Long
	...
	Dim y As Int = Abs(Years)
	Dim d As String = Format(y, 4)
	d = d & Format(Months, 2)
	d = d & Format(Days, 2)
	d = GetEra(Years < 0) & d
	Dim t As String = Format(Hours, 2)
	t = t & Format(Minutes, 2) 
	t = t & Format(Seconds, 2)
	Try
...
Public Sub SetDateAndTime2(Years As Int, Months As Int, Days As Int, Hours As Int, Minutes As Int, Seconds As Int, TimeZone As Double) As Long
	...
	Else 
		tzSign = "-"
	End If
	d = d & Format(Hours, 2) & Format(Minutes, 2) & Format(Seconds, 2) & "GMT" & tzSign & Format(tzHours, 2) & ":" & Format(tzMinutes Mod 60, 2)
	Try
...
Private Sub Format(Value As Int, lLength As Int) As String
	Return NumberFormat2(Value, lLength, 0, 0, False)
End Sub
