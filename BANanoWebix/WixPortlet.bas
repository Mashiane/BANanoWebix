...
'set padding
Sub SetPadding(padding As Object) As WixPortlet
	Portlet.SetPadding(padding)
	Return Me
End Sub

'set padding
Sub SetMargin(margin As Object) As WixPortlet
	Portlet.SetMargin(margin)
	Return Me
End Sub
...
Sub SetAttributes(m As Map) As WixPortlet
...
Sub SetStyles(m As Map) As WixPortlet
	For Each k As String In m.Keys
		Dim v As String = m.Get(k)
		Portlet.SetStyle(k,v)
	Next
	Return Me
End Sub
