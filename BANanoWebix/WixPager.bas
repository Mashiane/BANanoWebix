...
'set padding
Sub SetPadding(padding As Object) As WixPager
	Pager.SetPadding(padding)
	Return Me
End Sub

'set padding
Sub SetMargin(margin As Object) As WixPager
	Pager.SetMargin(margin)
	Return Me
End Sub

Sub SetAttributes(m As Map) As WixPager
...
Sub SetStyles(m As Map) As WixPager
	For Each k As String In m.Keys
		Dim v As String = m.Get(k)
		Pager.SetStyle(k,v)
	Next
