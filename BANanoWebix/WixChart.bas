...
Sub SetGradientFalling(r As String) As WixChart  'ignore
	Chart.SetProperty("gradient", "falling")
	Return Me
End Sub
...
Sub SetPadding(r As Int) As WixChart
	...
	Padding.Put("right", r)
	Padding.Put("left", r)
	Chart.setproperty("padding",Padding)
	Return Me
End Sub
...
'return the item
Sub Item As Map
	Chart.SetProperty("yAxis", yAxis)
	Chart.SetProperty("xAxis", xAxis)
	If ChartItem.Size > 0 Then
		Chart.SetProperty("item", ChartItem)
	End If
	If ChartLine.Size > 0 Then
		Chart.SetProperty("line", ChartLine)
	End If
	If Tooltip.Size > 0 Then
		Chart.SetProperty("tooltip", Tooltip)
	End If
	Return Chart.item
End Sub
