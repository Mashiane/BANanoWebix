...
Sub Init(pgContainer As String)
	...
	pg.Initialize("", pgContainer).SetHeader("Lesson 22: File Uploader - Part 3 (DropZone)(pgUploader3)").SetTypeSpace("").SetResponsive("master")
	'
	Dim R1 As WixRow
	R1.Initialize("R1")
	'
	Dim lst As WixList
	lst.Initialize("mylist").SetTypeUploader(True).SetHeight(600).SetWidth(600).SetScroll(False)
	'
	R1.AddItem(lst.Item)
	'
	pg.AddRow(R1)
