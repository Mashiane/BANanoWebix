...
Sub Init(pgContainer As String)
	...
	pg.Initialize("wp", pgContainer).SetHeader("Lesson 8.4 Editable Datatable (Use Tab) (pgLesson8_4)").SetResponsive("master")
	pg.SetLocale("ru-RU")
	'
	...
	dt.CreateColumn("title").SetHeaderTextFilter(True).SetSortString(True).SetTemplate(l).SetHeader("Film Title").SetEditorText("").SetWidth(200).AddToColumns(dt.DataTable)
	'
	dt.CreateColumn("year").SetHeaderSelectFilter(True).SetSortInt(True).SetEditorCombo("").SetHeader("Released").SetWidth(160).SetOptions(years).AddToColumns(dt.DataTable)
	'
	Dim obj As Map
